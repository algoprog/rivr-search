...
class Rivr{
	...
	public function doSearch($q){
		...
		foreach($results as $result){
			...
			if($result['magnet']) $magnet = " &middot; <a href=\"magnet:?xt=urn:btih:{$result['magnet']}\"><img src=\"images/magnet.png\" class=\"icon\"/></a>";
			else $magnet = '';
	
			if($result['type']!='other') $type = "<img src=\"images/{$result['type']}.png\" class=\"icon\"/>";
			else $type = '';
			...
			if($result['seeds'] && $result['peers']){
				$sp = "<span class=\"seeds\">{$result['seeds']}</span>/<span class=\"peers\">{$result['peers']}</span> &middot; ";
			}elseif($result['seeds']){
				$sp = "<span class=\"seeds\">{$result['seeds']}</span> &middot; ";
			}else{
				$sp = '';
			}
			
			if($result['age']){
				$age = " &middot; ".$result['age'];
			}else{
				$age = '';
			}
	
			$data = $data."<div class=\"result {$result['quality']}\">$type<a href=\"{$result['url']}\" target=\"blank\" class=\"title\">{$result['title']}</a><br/>$sp<span class=\"size\">{$result['size']}</span>$age &middot; {$result['source']}$magnet</div>";
	
			foreach($result['related'] as $subresult){
				if($subresult['magnet']) $magnet = " &middot; <a href=\"magnet:?xt=urn:btih:{$subresult['magnet']}\"><img src=\"images/magnet.png\" class=\"icon\"/></a>";
				else $magnet = '';
		
				if($subresult['type']!='other') $type = "<img src=\"images/{$subresult['type']}.png\" class=\"icon\"/>";
				else $type = '';
		
				$count[$subresult['type']]++;
				
				$quality[$subresult['quality']] = 1;
				
				if($subresult['seeds'] && $subresult['peers']){
					$sp = "<span class=\"seeds\">{$subresult['seeds']}</span>/<span class=\"peers\">{$subresult['peers']}</span> &middot; ";
				}elseif($subresult['seeds']){
					$sp = "<span class=\"seeds\">{$subresult['seeds']}</span> &middot; ";
				}else{
					$sp = '';
				}
				
				if($subresult['age']){
					$age = " &middot; ".$subresult['age'];
				}else{
					$age = '';
				}
		
				$data = $data."<div class=\"subresult {$result['quality']}\">$type<a href=\"{$subresult['url']}\" target=\"blank\" class=\"title\">{$subresult['title']}</a><br/>$sp<span class=\"size\">{$subresult['size']}</span>$age &middot; {$subresult['source']}$magnet</div>";
			}
		}
		$this->types = $count;
		...
		$count = $this->getTypes();
		arsort($count);
		$key = current(array_keys($count));
		$this->type = $this->getType($key);
		$total = $this->getTotal();
		if($total && $q && ($count['movie']/$total>0.75||$count['tv']/$total>0.75)){
			$this->imdb_id = $this->findImdbId($q);
		}
	}
	*/
}
